...
upacp_sdk_php
  ...
  │  ├生产环境配置文件
  │  │  │
  │  │  └readme.txt ┈┈┈┈┈┈┈┈┈生产环境配置文件样例（使用的时候在Web.config新增或者修改为此文件中的配置信息）
  │  │ 
  │  └对账文件样例 
  │     │
  ...
  │  
  ├upacp_demo_b2c.com.unionpay.sdk┈┈┈┈┈┈sdk
  │  │
  ...
  │  ├CertUtil.cs ┈┈┈┈┈ 证书工具类
  │  │
  │  ├HttpClient.cs ┈┈┈┈┈ http工具类
  │  │
  │  ├SDKConfig.cs ┈┈┈┈┈┈┈ 配置工具类
  │  │
  │  ├SDKUtil.cs ┈┈┈┈┈┈┈ sdk工具类
  │  │    
  │  └SecurityUtil.cs ┈┈┈┈┈┈┈安全相关工具类
  │  
...
2.【关于证书文件】测试环境使用的签名私钥证书，验签公钥证书均在assets目录下找得到，使用的时候只需要配置到Web.config指定的目录下即可，使用我们的demo不需要了解签名，验签等算法，详细参考配置文件注释。
...
5. 如需自行建立工程，流程请参考：https://open.unionpay.com/ajweb/help/faq/list?ha=%E3%80%90c#%E3%80%91%E5%A6%82%E4%BD%95%E5%BB%BA%E7%AB%8B%E5%B7%A5%E7%A8%8B%0A
