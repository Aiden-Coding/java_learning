 ...
 * 操作的性能对你来说是重要的，那么请不要将map的初始容量设置的过大(或者装填因子设置得 过低)。
 *
 * 
 * <p>
 ...
 * 
 * 本实现不是同步的。这意味着如果有多个线程同时访问该map，并且其中有至少一个 线程会修改map，那么就必须在map外部进行线程同步。
 *
 * 
...
public class JdkHashMap<K, V> extends AbstractMap<K, V>implements Map<K, V>, Cloneable, Serializable {
	 ...
	 */
	public Object clone() {
		JdkHashMap<K, V> result = null;
		try {
			result = (JdkHashMap<K, V>) super.clone();
		...
		if (result.table != EMPTY_TABLE) {
			result.inflateTable(Math.min((int) Math.min(size * Math.min(1 / loadFactor, 4.0f),
					// we have limits...
					JdkHashMap.MAXIMUM_CAPACITY), table.length));
		}
	...
	private abstract class HashIterator<E> implements Iterator<E> {
		...
		public void remove() {
			...
			current = null;
			JdkHashMap.this.removeEntryForKey(k);
			expectedModCount = modCount;
		}
	}
	...
	private final class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return JdkHashMap.this.removeEntryForKey(o) != null;
		}

		public void clear() {
			JdkHashMap.this.clear();
		}
	}
